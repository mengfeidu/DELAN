...
def get_tokenizer(args):
    ...
    if args.dataset == 'rxr' or args.tokenizer == 'xlm' or args.dataset == 'landrxr':
        cfg_name = 'xlm-roberta-base'
        tokenizer = AutoTokenizer.from_pretrained('../tokenizer_files/'+cfg_name)
    else:
...
def get_vlnbert_models(args, config=None):
    ...
    if args.dataset == 'rxr' or args.tokenizer == 'xlm' or args.dataset == 'landrxr':
        cfg_name = 'xlm-roberta-base'
        vis_config = PretrainedConfig.from_pretrained('../tokenizer_files/'+cfg_name)
    else:
        cfg_name = 'bert-base-uncased'
        vis_config = PretrainedConfig.from_pretrained('../tokenizer_files/'+cfg_name)
