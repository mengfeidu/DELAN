...
def build_dataset(args, rank=0, is_test=False):
    ...
    if args.test or (args.dataset != 'r4r' and args.dataset != 'landr4r'):
    ...
    if args.submit:
        if args.dataset == 'r2r' or args.dataset == 'cvdn':
            val_env_names.append('test')
        elif args.dataset == 'rxr' or args.dataset == 'landrxr':
...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    if args.dataset == 'r4r' or args.dataset == 'landr4r':
...
def main():
    args = parse_args()
    if args.use_mem_bank and args.world_size>1:
        raise Exception("current memory bank doesn't support multiply GPUs!")
    if args.world_size > 1:
