...
def build_dataset(args, rank=0, is_test=False):
    ...
    if args.aug is not None:
        ...
        # feat_db_aug = ImageFeaturesDB(args.img_ft_file, args.image_feat_size, args.img_ft_file_sd, args.aug_prob, aug_dataset=args.sepenv, partial_aug = args.partial_aug)
    ...
    if (args.dataset == 'r4r' or args.dataset == 'landr4r')  and (not args.test):
        val_env_names[-1] = 'val_unseen_sampled'
    
    if args.submit and args.dataset != 'r4r' and args.dataset != 'landr4r':
...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    if (args.dataset == 'r4r' or args.dataset == 'landr4r') and (not args.test):
        best_val = {'val_unseen_sampled': {"spl": 0., "sr": 0., "sr+spl": 0., "state":["","",""]}}
    else:
        best_val = {'val_unseen': {"spl": 0., "sr": 0., "sr+spl": 0., "state":["","",""]}}
    ...
    for idx in range(start_iter, start_iter+args.iters, args.log_every):
            ...
            if default_gpu:
                ...
                if env_name in best_val:
                    if score_summary['spl'] >= best_val[env_name]['spl']:
                        ...
                        best_val[env_name]['state'][0] = 'Iter %d %s' % (iter, loss_str)
                        listner.save(idx, os.path.join(args.ckpt_dir, "best_%s_spl" % (env_name)))

                    if score_summary['sr'] >= best_val[env_name]['sr']:
                        best_val[env_name]['sr'] = score_summary['sr']
                        best_val[env_name]['state'][1] = 'Iter %d %s' % (iter, loss_str)
                        listner.save(idx, os.path.join(args.ckpt_dir, "best_%s_sr" % (env_name)))

                    if score_summary['sr'] + score_summary['spl'] >= best_val[env_name]['sr+spl']:
                        best_val[env_name]['sr+spl'] = score_summary['sr'] + score_summary['spl']
                        best_val[env_name]['state'][2] = 'Iter %d %s' % (iter, loss_str)
                        listner.save(idx, os.path.join(args.ckpt_dir, "best_%s_both" % (env_name)))
        ...
        if default_gpu:
            ...
            for env_name in best_val:
                write_to_record_file(env_name + ' | ' + "\n".join(best_val[env_name]['state']), record_file)
