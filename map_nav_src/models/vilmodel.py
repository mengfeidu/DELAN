...
class ClsPredictionLarge(nn.Module):
    def __init__(self, hidden_size, input_size=None):
        super().__init__()
        if input_size is None:
            input_size = hidden_size
        self.net = nn.Sequential(nn.Linear(input_size, hidden_size * 2),
                                 nn.ReLU(),
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    ...
    def forward_landmark(self, txt_ids, txt_masks):
        txt_token_type_ids = torch.zeros_like(txt_ids)
        txt_embeds = self.embeddings(txt_ids, token_type_ids=txt_token_type_ids)
        txt_embeds = self.land_encoder(txt_embeds, txt_masks)
        return txt_embeds

    def forward_panorama_per_step(
        self, view_img_fts, obj_img_fts, loc_fts, nav_types, view_lens, obj_lens,        
        tcl_hist_weight, tcl_obs_weight, max_obs_num
    ):
        ...
        if tcl_hist_weight > 0 or tcl_obs_weight > 0:
            pano_masks = gen_seq_masks(pano_lens, max_len=max_obs_num-1) # pano_embeds not have [stop]
        else:
    ...
    def forward_navigation_per_step(
        ...
        tcl_hist_weight, tcl_obs_weight
    ):
        ...
        ## cl modified
        if tcl_hist_weight > 0 or tcl_obs_weight > 0:
            cl_gmap_embeds = gmap_embeds.clone()
        else:
            cl_gmap_embeds = None
        ##
        ...
        outs = {
            ...
            'cl_gmap_embeds':cl_gmap_embeds,
            'cl_vp_embeds':cl_vp_embeds,
    ...
    def forward(self, mode, batch, **kwargs):
        ...
        if mode == 'landmark':
            land_embeds = self.forward_landmark(batch['txt_ids'], batch['txt_masks'])
            return land_embeds
            ...
            pano_embeds, pano_masks = self.forward_panorama_per_step(
                    ...
                    batch['tcl_hist_weight'], batch['tcl_obs_weight'], batch['max_obs_num']
                )
            return pano_embeds, pano_masks
            ...
            return self.forward_navigation_per_step(
                ...
                batch['tcl_hist_weight'], batch['tcl_obs_weight']
            )
